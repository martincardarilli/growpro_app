...
const CreateAutomation = () => {
  ...
  const calculateTotalTime = (start, end) => {
    ...
    const totalMinutes =
      endTime >= startTime ? endTime - startTime : 1440 - startTime + endTime; // 1440 = 24 * 60

    return totalMinutes;
  };

  const totalTimeInterval = calculateTotalTime(
    form.horaEncendido,
    form.horaApagado
  );
      ...
      <ScrollView>
        ...
        <View className="mt-5">
          <Text className="text-lg text-white font-semibold">
            Tiempo intervalo total: {Math.floor(totalTimeInterval / 60)} horas y{" "}
            {totalTimeInterval % 60} minutos
          </Text>
        </View>
