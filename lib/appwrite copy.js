    ...
    const automatizacionesWithDevices = await Promise.all(
      automatizaciones.documents.map(async (automatizacion) => {
       // const deviceMac = automatizacion.device;
        const deviceMac = automatizacion.device.$id;

        console.log("Automatización Device MAC:", deviceMac); // Debug: check if the device ID exists
        ...
        if (deviceMac) {
          try {
            ...
            const devices = await databases.listDocuments(
              ...
              [Query.equal("$id", deviceMac)] // Ensure you're searching by the correct field
            );

            // Si se encuentra el dispositivo, devolver los detalles
            const device = devices.documents[0] || null;
            console.log("Device Found:", device); // Debug: check if the device was found
            
            return {
              ...automatizacion,
              model: device ? device.model : 'Unknown1',
              name: device ? device.name : 'Unknown1',
            };
          } catch (error) {
